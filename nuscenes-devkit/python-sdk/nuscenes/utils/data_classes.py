...
class PointCloud(ABC):
    ...
    @classmethod
    def from_file_multisweep_bf(cls,
                                nusc: 'NuScenes',
                                sample_rec: Dict,
                                chan: str,
                                ref_chan: str,
        ...
        :param nusc: A NuScenes instance.
        :param sample_rec: The current sample.
        :param chan: The channel from which we track back n sweeps to aggregate the point cloud.
        :param ref_chan: The reference channel of the current sample_rec that the point clouds are mapped to.
        ...
        :param min_distance: Distance below which points are discarded.
        :param use_keyframe: Whether to use only keyframe, or use the sensor sweeps (may not be keyframe)
        :return: (all_pc, all_times). The aggregated point cloud and timestamps.
        """
        ...
        for _ in range(nsweeps_back):
            ...
            if use_keyframe:
                if iter_sample_rec['prev'] == '':  # Abort if there are no previous sweeps.
                    break
                else:
                    iter_sample_rec = nusc.get('sample', iter_sample_rec['prev'])
                    current_sd_rec = nusc.get('sample_data', iter_sample_rec['data'][chan])
            else:
        ...
        if (use_keyframe and iter_sample_rec['next'] == '') or (not use_keyframe and current_sd_rec['next'] == ''):
            return all_pc, np.squeeze(all_times, axis=0)
        else:
            if use_keyframe:
                iter_sample_rec = nusc.get('sample', iter_sample_rec['next'])
                current_sd_rec = nusc.get('sample_data', iter_sample_rec['data'][chan])
            else:
        ...
        for _ in range(nsweeps_forward):
            ...
            if use_keyframe:
                if iter_sample_rec['next'] == '':  # Abort if there are no future sweeps.
                    break
                else:
                    iter_sample_rec = nusc.get('sample', iter_sample_rec['next'])
                    current_sd_rec = nusc.get('sample_data', iter_sample_rec['data'][chan])
    ...
    @classmethod
    def from_file_multisweep_bf_icp(cls,
                                    nusc: 'NuScenes',
                                    sample_rec: Dict,
                                    chan: str,
                                    ref_chan: str,
        ...
        As every sweep is in a different coordinate frame, we need to map the coordinates to a single reference frame.
        As every sweep has a different timestamp, we need to account for that in the transformations and timestamps.
        The transformation is estimated with icp.
        :param nusc: A NuScenes instance.
        :param sample_rec: The current sample.
        :param chan: The channel from which we track back n sweeps to aggregate the point cloud.
        :param ref_chan: The reference channel of the current sample_rec that the point clouds are mapped to.
        ...
        for i in range(nsweeps_back):
            ...
            current_pc = cls.from_file(osp.join(nusc.dataroot, current_sd_rec['filename']))
            if i == 0:
                ref_points_xyz = current_pc.points[0:3, :]
                ref_points_xyz = ref_points_xyz.T

                ref_points = o3d.geometry.PointCloud()
                ref_points.points = o3d.utility.Vector3dVector(ref_points_xyz)
            ...
            # estimate the transformation matrix with ICP
            if i != 0:
                current_points_xyz = current_pc.points[0:3, :]
                current_points_xyz = current_points_xyz.T

                current_points = o3d.geometry.PointCloud()
                current_points.points = o3d.utility.Vector3dVector(current_points_xyz)

                reg_p2p = o3d.registration.registration_icp(
                    current_points, ref_points, 0.05, np.eye(4), o3d.registration.TransformationEstimationPointToPoint())

                # import copy
                # source_temp = copy.deepcopy(current_points)
                # target_temp = copy.deepcopy(ref_points)
                # source_temp.paint_uniform_color([1, 0.706, 0])
                # target_temp.paint_uniform_color([0, 0.651, 0.929])
                # source_temp.transform(reg_p2p.transformation)
                # o3d.visualization.draw_geometries([source_temp, target_temp])

                icp_trans_matrix = reg_p2p.transformation
                current_pc.transform(icp_trans_matrix)
    ...
    def from_file_multisweep_bf_sample_data(cls,
        ...
        all_pc = cls(points)
        all_times = np.zeros((1, 0))
        all_pc_future = cls(points)
        all_times_future = np.zeros((1, 0))
        all_pc_past = cls(points)
        all_times_past = np.zeros((1, 0))
        ...
        for k in range(1, nsweeps_forward + 1):
            future_sample_data_token = current_sd_rec['token']
            future_sample_token = current_sd_rec['sample_token']
            future_is_key_frame = current_sd_rec['is_key_frame']
            # Load up the pointcloud.
            current_pc = cls.from_file(osp.join(nusc.dataroot, current_sd_rec['filename']))
            ...
            all_times_future = np.hstack((all_times_future, times))

            # Merge with key pc.
            if k % (skip_frame + 1) == 0:
                all_pc_future.points = np.hstack((all_pc_future.points, current_pc.points))
            else:
                tmp_points = np.zeros((4, 1), dtype=np.float32)
                all_pc_future.points = np.hstack((all_pc_future.points, tmp_points))  # dummy value
        ...
        for k in range(nsweeps_back):
            ...
            if k % (skip_frame + 1) == 0:
                ...
                times = time_lag * np.ones((1, 1))  # dummy value
            all_times_past = np.hstack((all_times_past, times))

            # Merge with key pc.
            if k % (skip_frame + 1) == 0:
                all_pc_past.points = np.hstack((all_pc_past.points, current_pc.points))
            else:
                tmp_points = np.zeros((4, 1), dtype=np.float32)
                all_pc_past.points = np.hstack((all_pc_past.points, tmp_points))  # dummy value
        ...
        all_pc.points = np.hstack((all_pc_past.points,all_pc_future.points))
        all_times = np.hstack((all_times_past, all_times_future))

        if return_trans_matrix:
            return all_pc, np.squeeze(all_times, 0), trans_matrix_list
        else:
            return all_pc, np.squeeze(all_times, 0),future_sample_data_token,future_sample_token,future_is_key_frame

    @classmethod
    def get_instance_boxes_multisweep(cls,
                                      nusc: 'NuScenes',
                                      sample_rec: Dict,
                                      instance_token: str,
                                      ref_chan: str,
                                      use_keyframe: bool = False,
                                      nsweeps_back: int = 5,
                                      nsweeps_forward: int = 5) -> Tuple[List['Box'], np.array, List[str], List[str]]:
        """
        ...
        :param ref_chan: The reference channel of the current instance_rec that the bounding boxes are mapped to.
        :param use_keyframe: Whether to use only keyframe, or use the sensor sweeps (may not be keyframe)
        :param nsweeps_back: Number of sweeps to aggregate. The sweeps trace back.
        :param nsweeps_forward: Number of sweeps to aggregate. The sweeps are obtained from the future.
        ...
        iter_sample_rec = sample_rec
        sample_data_token = sample_rec['data'][ref_chan]
        current_sd_rec = nusc.get('sample_data', sample_data_token)
        for _ in range(nsweeps_back):
            ...
            if use_keyframe:
                if iter_sample_rec['prev'] == '':
                    break
                else:
                    iter_sample_rec = nusc.get('sample', iter_sample_rec['prev'])
                    current_sd_rec = nusc.get('sample_data', iter_sample_rec['data'][ref_chan])
            else:
                if current_sd_rec['prev'] == '':
                    break
                else:
    ...
    def get_instance_boxes_multisweep_sample_data(cls,
        ...
        current_sd_rec = ref_sd_rec
        for _ in range(nsweeps_back):
            box, attr, cat = nusc.get_instance_box(current_sd_rec['token'], instance_token) # sample_data_token and instance_token confirm the box
            boxes.append(box)  # It is possible the returned box is None
            attr_list.append(attr)
            cat_list.append(cat)
    ...
    @classmethod
    def get_all_boxes_multisweep(cls,
                                 nusc: 'NuScenes',
                                 sample_rec: Dict,
                                 ref_chan: str,
                                 use_keyframe: bool = False,
                                 nsweeps_back: int = 5,
                                 nsweeps_forward: int = 5) -> Tuple[List[List['Box']], np.array]:
        """
        Return all the bounding boxes across different sweeps.
        This function is for visualization and thus debugging purpose.
        For each bounding box, we need to map its (global) coordinates to the reference frame.
        :param nusc: A NuScenes instance.
        :param sample_rec: The current sample.
        :param ref_chan: The reference channel of the current instance_rec that the bounding boxes are mapped to.
        ...
        :param nsweeps_back: Number of sweeps to aggregate. The sweeps trace back.
        :param nsweeps_forward: Number of sweeps to aggregate. The sweeps are obtained from the future.
        :return: (list of bounding boxes, the time stamps of bounding boxes)
        """
        ...
        current_sd_rec = nusc.get('sample_data', sample_data_token)
        for _ in range(nsweeps_back):
            boxes = nusc.get_boxes(current_sd_rec['token'])
            boxes_list.append(boxes)  # It is possible the returned box is None
        ...
        for boxes in boxes_list:
            ...
            res_boxes_list.append(res_boxes)

        return res_boxes_list, all_times
...
class Box:
    ...
    def render_vis(self,
        ...
        if cat.startswith('vehicle'):
            color = 'royalblue'
        elif cat == 'human.pedestrian.adult':
            color = 'c'
        elif cat == 'human.pedestrian.wheelchair':
            color = 'r'
        else:
            color = 'darkorange'

        if cat.startswith('movable'):
            return
        ...
        axis.plot([corners.T[0][0], corners.T[1][0]],
                  [corners.T[0][1], corners.T[1][1]],
                  color=color, linewidth=linewidth)

        axis.plot([corners.T[4][0], corners.T[5][0]],
                  [corners.T[4][1], corners.T[5][1]],
                  color=color, linewidth=linewidth)
